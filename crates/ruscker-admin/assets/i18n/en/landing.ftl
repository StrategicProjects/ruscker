...
admin-blocks-subtitle = HTML snippets rendered on the landing (top / bottom).
...
admin-users-kpi-admins = Admins
admin-users-kpi-editors = Editors
admin-users-kpi-viewers = Viewers
admin-users-kpi-password-change = Password change pending
admin-users-edit = Edit user
admin-users-edit-title = Edit user
admin-users-edit-subtitle = Update access and profile in one save.
admin-users-account = Account details
admin-users-save = Save changes
admin-users-cancel = Cancel
admin-users-password-section = Password reset
admin-users-password-reset-hint = Sets a temporary password and requires the user to change it at the next sign-in.
...
admin-users-export-all = Export CSV (all)
admin-users-export-filtered = Export filtered
...
admin-users-setor = Department
admin-users-setor-placeholder = e.g. GAPE
...
admin-users-import-status-ok = will import
admin-users-import-status-exists = exists — skipped
admin-users-import-status-bad-username = invalid username
admin-users-import-status-bad-password = password fails the policy (min 8, with upper, lower, digit and special)
admin-users-import-status-bad-role = invalid role
admin-users-import-confirm = Import users
admin-users-import-cancel = Cancel
admin-users-import-done-prefix = Imported:
...
admin-chart-expand = Expand chart
admin-chart-close = Close
admin-chart-no-data = Not enough data yet
admin-chart-access-title = Accesses per day
admin-chart-period-14 = 14 days
admin-chart-period-30 = 30 days
admin-chart-period-90 = 90 days
admin-chart-cpu = CPU
admin-chart-mem = Memory
admin-chart-window = last
...
admin-specs-kpi-active = Active
admin-specs-kpi-archived = Archived
admin-specs-kpi-public = Public
admin-specs-kpi-restricted = Restricted
...
admin-specs-update-image = Update image (re-pull)
admin-specs-update-image-running = Updating image…
admin-specs-update-image-ok = Image updated
admin-specs-update-image-fail = Image update failed
...
spec-form-created-title = App created
spec-form-created-body = The app was created successfully. What would you like to do next?
spec-form-created-stay = Back to the form
spec-form-created-list = Go to the app list
spec-form-kind = Kind
...
spec-form-image-repull = Update image
spec-form-image-repull-help = Re-pull from the registry — use after re-publishing the same tag (changed bytes or CPU architecture).
...
admin-images-kpi-files = Files
admin-images-kpi-in-use = In use
admin-images-kpi-total-size = Total size
...
admin-audit-kpi-events = Events
admin-audit-kpi-users = Distinct users
...
admin-landing-style = Custom CSS
...
admin-landing-custom-css-help = Injected at the end of the public portal. Use with care.
...
admin-landing-logo-main = Main logo (header)
admin-landing-logo-main-help = "Mark + name" uses the Ruscker symbol; "Symbol only" hides the title; "Custom" replaces the symbol with your image. Size and margin below apply to both.
admin-landing-logos-extra = Additional logos
admin-landing-logos-extra-help = Extra logos in the header center/right or in the footer (partners, institutions). The header's left side belongs to the main logo.
admin-landing-header-style-card = Header style
admin-landing-bgmode-preset = Preset
admin-landing-bgmode-help = Preset uses the built-in styles; Solid and Gradient paint a custom background that replaces the preset.
admin-landing-header-dark-inherit = inherits light
admin-landing-header-dark-inherit-help = Left blank, the dark theme uses the same background as the light one.
admin-landing-cards-card = Catalog cards
admin-landing-theme-card = Theme & colors
...
admin-disk-kpi-containers = Containers
admin-disk-kpi-stopped = Stopped
admin-disk-kpi-images = Images
admin-disk-kpi-unused = Unused images
...
admin-disk-containers-unavailable = Docker's container inventory couldn't be loaded. This is a partial view; retry once Docker responds.
...
admin-disk-images-unavailable = Docker's image inventory couldn't be loaded. This is a partial view; retry once Docker responds.
...
admin-disk-usage-unknown = Docker couldn't be queried for running containers — every image is shown as in-use and removal is disabled to avoid deleting an image still in use. Retry once Docker responds.
admin-disk-in-use-hint = In use — can't be removed.
admin-disk-foreign = not managed
admin-disk-foreign-hint = Not a Ruscker image (e.g. another app on this host) — Ruscker won't remove it.
...
admin-disk-volumes-hint = Named Docker volumes on this host. Only volumes created by Ruscker and referenced by nothing can be removed.
admin-disk-volumes-create = Create
admin-disk-volumes-name-placeholder = volume-name
admin-disk-volumes-empty = No named volumes.
admin-disk-volumes-unavailable = The volume inventory couldn't be loaded (or this backend doesn't manage volumes). This is a partial view; retry once Docker responds.
admin-disk-volumes-locked = In use, referenced by an app, or not created by Ruscker — it won't be removed from here.
admin-disk-volumes-badge-ruscker = Ruscker
admin-disk-volumes-badge-external = external
admin-disk-volumes-confirm-remove = Remove this volume? Its DATA is deleted permanently — this cannot be undone.
admin-disk-volumes-col-name = Volume
admin-disk-volumes-col-driver = Driver
admin-disk-volumes-col-created = Created
admin-disk-volumes-col-refs = References
admin-disk-volumes-col-origin = Origin
admin-disk-flash-volume-created = Volume created.
admin-disk-flash-volume-removed = Volume removed.
admin-disk-flash-volume-bad-name = Invalid volume name — use letters, digits, "_", "." or "-", starting with a letter or digit.
...
admin-landing-header-bg-preset = Header style
admin-landing-header-bg-preset-help = A custom background color (under Appearance) overrides this preset.
admin-landing-preset-flat = Flat
admin-landing-preset-soft = Soft
admin-landing-preset-bold = Bold
admin-landing-card-cover-default = Default card cover
admin-landing-cover-auto = Auto
admin-landing-cover-auto-sub = type colour
admin-landing-cover-own = Own
admin-landing-cover-inherited = Inherited
admin-landing-cover-inherits-line = Inherits the light theme's background.
admin-landing-card-cover-default-auto-help = Each card takes a tint of its type's colour. No configuration — it adapts automatically.
...
admin-system-kpi-specs = Apps
admin-system-kpi-replicas = Replicas
admin-system-kpi-docker = Docker
admin-system-kpi-db = Database
...
admin-schedules-kpi-active = Active
admin-schedules-kpi-recent-failures = Recent failures
...
admin-mfa-recovery-regenerated-title = Your new recovery codes
admin-mfa-recovery-download = Download (.txt)
admin-mfa-recovery-copy = Copy all
admin-mfa-recovery-copied = Copied
admin-mfa-recovery-file-header = Ruscker — 2FA recovery codes
admin-mfa-recovery-file-user = User
admin-mfa-recovery-file-generated = Generated at
admin-mfa-recovery-file-note = Each code works exactly once. Keep this file somewhere safe and remove it from shared folders.
admin-mfa-recovery-remaining = Recovery codes left:
admin-mfa-recovery-low = Few recovery codes remain. Generate a new set before the last one is used.
admin-mfa-recovery-regenerate = Generate new codes
admin-mfa-recovery-regenerate-help = The current codes stop working immediately. Confirm your password to generate a fresh set of 10 codes.
admin-mfa-recovery-regenerate-confirm = Generating new codes invalidates every current recovery code. Continue?
admin-mfa-error-stale = This page was out of date: the current codes were left unchanged. Reload and try again.
admin-mfa-recovery-copy-failed = Automatic copy was refused. Select the codes and copy them by hand.
...
admin-activity-kpi-app-accesses = App accesses
admin-activity-kpi-users = Distinct users
