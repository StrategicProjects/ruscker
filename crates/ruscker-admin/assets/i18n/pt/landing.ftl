...
admin-chart-expand = Expandir gráfico
admin-chart-close = Fechar
admin-chart-no-data = Ainda sem dados suficientes
admin-chart-access-title = Acessos por dia
admin-chart-period-14 = 14 dias
admin-chart-period-30 = 30 dias
admin-chart-period-90 = 90 dias
admin-chart-cpu = CPU
admin-chart-mem = Memória
...
spec-form-error-editor-scope = Um app restrito deve manter pelo menos um dos seus grupos e não pode incluir grupos fora do seu escopo de Editor.
...
admin-disk-kpi-stopped = Parados
admin-disk-kpi-images = Imagens
admin-disk-kpi-unused = Imagens sem uso
...
admin-system-kpi-version = Versão
admin-system-kpi-specs = Aplicações
admin-system-kpi-replicas = Réplicas
admin-system-kpi-docker = Docker
admin-system-kpi-db = Banco
...
admin-mfa-recovery-regenerated-title = Seus novos códigos de recuperação
admin-mfa-recovery-download = Baixar (.txt)
admin-mfa-recovery-copy = Copiar todos
admin-mfa-recovery-copied = Copiado
admin-mfa-recovery-file-header = Ruscker — códigos de recuperação do 2FA
admin-mfa-recovery-file-user = Usuário
admin-mfa-recovery-file-generated = Gerado em
admin-mfa-recovery-file-note = Cada código funciona uma única vez. Guarde este arquivo em um local seguro e apague-o de pastas compartilhadas.
admin-mfa-recovery-remaining = Códigos de recuperação restantes:
admin-mfa-recovery-low = Restam poucos códigos de recuperação. Gere um novo conjunto antes que o último seja usado.
admin-mfa-recovery-regenerate = Gerar novos códigos
admin-mfa-recovery-regenerate-help = Os códigos atuais deixam de valer imediatamente. Confirme sua senha para gerar um novo conjunto de 10 códigos.
admin-mfa-recovery-regenerate-confirm = Gerar novos códigos invalida todos os códigos de recuperação atuais. Continuar?
admin-mfa-error-stale = Esta página estava desatualizada: os códigos atuais não foram alterados. Recarregue e tente de novo.
admin-mfa-recovery-copy-failed = Não foi possível copiar automaticamente. Selecione os códigos e copie à mão.
