...
admin-landing-card-cover-default-auto-help = Auto keeps each card's per-type tint; pick a colour or gradient to set one default cover for the whole catalog (cards with their own cover/accent still win).
